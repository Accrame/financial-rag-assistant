...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.documents_loaded:
            st.markdown(
                f'<p class="status-success">✓ {st.session_state.document_count} chunks indexed</p>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                '<p class="status-warning">⚠ No documents loaded</p>', unsafe_allow_html=True
            )

        # API status
        if os.getenv("OPENAI_API_KEY"):
            st.markdown(
                '<p class="status-success">✓ OpenAI API configured</p>', unsafe_allow_html=True
            )
        elif os.getenv("ANTHROPIC_API_KEY"):
            st.markdown(
                '<p class="status-success">✓ Anthropic API configured</p>', unsafe_allow_html=True
            )
        else:
            st.markdown(
                '<p class="status-error">✗ No LLM API key found</p>', unsafe_allow_html=True
            )
